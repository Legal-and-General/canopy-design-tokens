    ...
    transforms: {
      'size/pxToRem': {
        ...
        filter: function(token) {
          ...
          if ((pathString.includes('-cols-') || pathString.endsWith('-cols')) ||
      ...
      'asset/fontFamily': {
        ...
        filter: function(token) {
          return token.type === 'fontFamily' || 
                 (token.path && token.path.some(p => p.toLowerCase().includes('font-family')));
        },
        transform: function(token) {
      ...
      'size/letterSpacingRound': {
        type: 'value',
        filter: function(token) {
          return token.path && token.path.some(p => p.toLowerCase().includes('letter-spacing'));
        },
        transform: function(token) {
          const value = token.value;
      ...
      files: [{
        ...
        filter: function(token) {
          return token.value !== null && 
                 ...
                 (token.filePath.includes('component-themes') || token.filePath.includes('colour.json'));
        },
        options: {
